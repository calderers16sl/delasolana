...
- **Language switcher**: `src/components/ui/LanguageSwitcher.vue` — accepts `placement="desktop"` (globe icon + popover, used in the nav) or `placement="mobile"` (inline language list shown inside the existing mobile menu). Uses endonyms (`Català`, `Español`, `English`, `Français`, `日本語`, `中文`).
